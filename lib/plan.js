	...
	for (const configuration of attemptConfigurations(input, configuredRetention)) {
		...
		if (attempt.kind === 'span') {
			return {
				...
				...(configuration.allowHead === true ? { emergencyHead: true } : {}),
 ...
 *    as a pair).
 *
 * When `emergency` is set, one more configuration follows: the head-protected
 * nodes (the original task statement) become elidable too. It exists for the case
 * where compaction has already failed and the choice is "drop the statement" or
 * "lose the turn". The newest human instruction stays a hard barrier regardless.
 *
...
function attemptConfigurations(input, configuredRetention) {
	...
	if (input.emergency === true) tiers.push({ retainTokens: configuredRetention, tailHard: false, allowFinalNode: true, allowHead: true });
	return tiers;
}
...
function attemptSpan(input) {
	...
	// `allowHead` is the emergency relaxation: index 0 becomes elidable (a barrier
	// such as the system prompt is still skipped by elidableRegions).
	const headEnd = input.allowHead === true ? 0 : protectedHeadEnd(nodes, input.protectHeadNodes);
