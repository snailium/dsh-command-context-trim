...
export function registerAutoTrim(ctx, config) {
	...
	ctx.on(
		...
		async ({ agent, failure, signal }, next) => {
			process.stderr.write(`[ctx-trim-probe] handler code=${failure?.code} aborted=${signal.aborted}\n`);
			...
			try {
				...
				process.stderr.write(`[ctx-trim-probe] threw: ${describeError(error)}\n`);
			...
			if (agent.session.surface.replaceGeneration <= generation) {
				process.stderr.write(`[ctx-trim-probe] no-op: ${outcome.result.text.split('\n')[0]}\n`);
				log(ctx, 'info', `context-overflow auto-trim: nothing safely trimmable; leaving recovery to compaction`);
				return next();
			}
			process.stderr.write(`[ctx-trim-probe] TRIMMED plan=${outcome.plan.shadowedSeqs.length} nodes, after=${outcome.after.totalTokens}\n`);
