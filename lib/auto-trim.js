...
export function registerAutoTrim(ctx, config) {
	...
	ctx.on(
		...
		async ({ agent, failure, signal }, next) => {
			...
			let outcome;
			try {
				outcome = await executeTrim(ctx, config, { agent, signal, routedOnly: true, budgetCeiling });
			} catch (error) {
			...
			log(ctx, 'warn', `context-overflow auto-trim: ${logLine(outcome.plan, outcome.after, outcome.label)}`);
			return { kind: 'retry' };
		},
		{ prepend: true }
	);
