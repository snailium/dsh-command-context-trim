 ...
 *
 * When it cannot trim, it does not simply hand the wall to compaction: it *wraps*
 * compaction's own listener by calling `next()`, and if that comes back with no
 * action — compaction failed to recover and the turn is about to die with the
 * original error — it takes one last-resort (emergency) trim instead. That is the
 * only case where the head-protected task statement may be elided; the newest human
 * instruction is never crossed.
 *
 ...
 */
export function registerAutoTrim(ctx, rawConfig) {
	// Re-read per event: `compactionTargetRatio`/`compactionRoute` are volatile
	// handles, and the settings card may change them while the session runs.
	...
	/** agents that already used their one emergency trim this episode. */
	const emergencySpent = new WeakSet();
	...
	ctx.on(
		...
		async ({ agent, failure, signal }, next) => {
			...
			/**
			 * Hand the wall to compaction, then check whether it actually recovered.
			 * `next()` runs the rest of the waterfall and resolves to whatever the
			 * downstream handler returned: a value means compaction pruned and/or
			 * summarized and the loop is already retrying; `undefined` means the
			 * original error is about to be preserved and the turn will fail.
			 */
			const delegate = async () => {
				...
				try {
					...
					// Compaction threw on its way out; keep the behaviour of the
					// non-emergency path when the switch is off.
					if (current.emergencyTrim !== true) throw error;
					downstream = undefined;
				}
				if (downstream !== undefined || signal.aborted || current.emergencyTrim !== true) return downstream;
				if (emergencySpent.has(agent)) {
					log(ctx, 'warn', 'context-overflow auto-trim: compaction could not recover and the emergency trim is spent; preserving the original error');
					return downstream;
				}
				const generation = agent.session.surface.replaceGeneration;
				let outcome;
				try {
					outcome = await executeTrim(ctx, current, { agent, signal, routedOnly: true, inPlaceFirst: true, emergency: true });
				} catch (error) {
					log(ctx, 'warn', `context-overflow auto-trim: emergency trim failed (${describeError(error)}); preserving the original error`);
					return downstream;
				}
				if (signal.aborted) return downstream;
				if (agent.session.surface.replaceGeneration <= generation) {
					log(ctx, 'warn', 'context-overflow auto-trim: compaction could not recover and nothing else can be trimmed; preserving the original error');
					return downstream;
				}
				emergencySpent.add(agent);
				actors.set(agent.session, agent);
				log(ctx, 'warn', `context-overflow auto-trim: compaction could not recover; emergency ${logLineFor(outcome)}`);
				return { kind: 'retry' };
			};
			const current = config();
			const used = spent.get(agent) ?? 0;
			if (used >= current.maxAutoTrimRetries) {
			...
			const budgetCeiling =
				used === 0 ? undefined : Math.max(1, Math.floor(failingTotal * (1 - current.autoTrimShrink)));
			...
			let outcome;
			try {
				outcome = await executeTrim(ctx, current, { agent, signal, routedOnly: true, budgetCeiling, inPlaceFirst: true });
			} catch (error) {
