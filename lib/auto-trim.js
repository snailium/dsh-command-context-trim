		...
		async ({ agent, failure, signal }, next) => {
			...
			const delegate = async () => {
				...
				try {
					...
					if (config.emergencyTrim !== true) throw error;
					downstream = undefined;
				}
				if (downstream !== undefined || signal.aborted || config.emergencyTrim !== true) return downstream;
				...
				let outcome;
				try {
					outcome = await executeTrim(ctx, config, { agent, signal, routedOnly: true, inPlaceFirst: true, emergency: true });
				} catch (error) {
