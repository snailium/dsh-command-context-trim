 ...
 * the Host reports, and the cards a browser bundle registers under the same key.
 * The key is the **loader entry id** (`context-trim`, the row id our bundle patch
 ...
 *
 * This module renders only the form **body**: the platform supplies the frame (the
 * title from the slot's `label`, the disclosure, the artwork). Drawing our own card
 * around it produces a doubled, mis-styled card with a header button that swallows
 * the platform's clicks.
 *
 ...
 * Plugins page.
 *
 * @module dsh-command-context-trim/client
 */
import {
	SettingsForm,
	SettingsFormModel,
...
const en = {
	...
	compactionRouteHint: 'Optional provider:model used by a generated preset for compaction summarization. Leave empty to keep the route defaults.',
...
class TrimCardController {
	 ...
	 */
	constructor(scope) {
		this.form = new SettingsFormModel(scope, [settingsNumberField('compactionTargetRatio'), settingsTextField('compactionRoute')]);
...
export function apply(ctx) {
	// Older web hosts (0.1.2/0.1.5) serve `settingsScope` instead of `configForms`;
	// registering nothing is better than throwing inside their Plugins page.
