...
| Credentials | no | No `process.env`, no key, no token. Provider credentials stay with the `llm-pi-ai` row and are never read, logged or forwarded. |
...
/trim preset check        # print the generated row, write nothing
...
| Check | State |
|---|---|
| `npm test` (103 tests: planner, args, surface apply + log replay, plugin handler, automatic overflow path, in-place slim, config) | ✅ passing |
